...
class UserSs(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        depth = 2
        # User 0 -> UserProfile 1 -> User 2 xxx UserProfile 3
...
def getuser(request):
    ...
    serializers = UserSs(data=users, many=True)
...
# 获取用户列表的接口
# 使用上面的getuser可以获取数据，但是无token后面无法操作
@api_view(['GET'])
# 下面这句去掉也可获取数据，但是后面同样不能操作
    ...
    if serializers.is_valid():
        ...
        if user is not None:
            ...
            if user.is_superuser:
                ...
                body ={
                    'success':2,
                    'msg':"Sorry, You Can NOT Login This Site",
                }
                return Response(body, status=status.HTTP_403_FORBIDDEN)
        else:
            body ={
                'success':3,
                'msg':'user is not exist,please try again',
            }
